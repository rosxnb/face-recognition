...
def pre_process(img):
    img_hsv = cv.cvtColor(src=img, code=cv.COLOR_BGR2HSV)
    # img_gray = cv.cvtColor(src=img, code=cv.COLOR_BGR2GRAY)
    img_blur = cv.GaussianBlur(src=img_hsv, ksize=(3, 3), sigmaX=1)
    img_canny = cv.Canny(image=img_blur, threshold1=200, threshold2=200)

    kernel = np.ones( (5, 5) )
    img_dilated = cv.dilate(src=img_canny, kernel=kernel, iterations=4)
    img_eroded = cv.erode(src=img_dilated, kernel=kernel, iterations=2)
...
def get_contours(img, img_contours, threshold=3000):
    result = np.array([])
    max_area = 0
...
def get_warp(img, corners):
    ...
    #     20:img_output.shape[0]-20,
    #     20:img_output.shape[1]-20
    # ]
    #
    # img_cropped = cv.resize(img_cropped, (img_width, img_height))
    # return img_cropped
...
def real_time_scanner():
    ...
    while True:
        ...
        img_threshold = pre_process(img)
        detected_doc = get_contours(img_threshold, img_contour, threshold=5_000)

        if detected_doc.size != 0:
            img_warpped = get_warp(img, detected_doc)
            img_stack = ([img_contour, img_warpped, img_threshold])
            # cv.imshow("ImageWarped", img_warpped)

        else:
            img_stack = ([img, img_threshold, img_contour])
